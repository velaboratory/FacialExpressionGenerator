    ...
    if(newFrame):
        ...
        if(undistRect):
                ...
                for (LabelLeft, LabelRight) in  zip(prediction[0],prediction[1]):
                    ...
                    cv2.drawMarker(leftRightImage[0], ((LabelLeft[0]  *640).astype(int), (LabelLeft[1] * 480).astype(int)),
                    (200, 150, 150), markerType=cv2.MARKER_STAR,markerSize=2, thickness=2, line_type=cv2.LINE_AA)
                    cv2.drawMarker(leftRightImage[1], ((LabelRight[0]*640).astype(int), (LabelRight[1]*480).astype(int)), 
